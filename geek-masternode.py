...
port = "6889"
rpcuser = "geekuser"
datadir = "/root/.geekcash"
config_file = "geekcash.conf"
cli = "geekcash-cli"
coind = "geekcashd"
walletUrl = "https://github.com/GeekCash/geekcash/releases/download/v%s" % (fullVer)
walletFile = "geekcash-%s-x86_64-linux-gnu.tar.gz" % (walletVer)
ticker = "GEEK"
...
def removeOldFiles():
    ...
    subprocess.call("rm -rf %s /usr/local/bin/geek*" % (datadir), shell=True)
...
def procFile(data):
    ...
    subprocess.call("mv /tmp/geekcash*/bin/%s /tmp/geekcash*/bin/%s /usr/local/bin/" % (coind, cli), shell=True)
...
def installSentinel():
    ...
    subprocess.call("cd /root && git clone https://github.com/GeekCash/sentinel geek-sentinel", shell=True)
    subprocess.call("cd /root/geek-sentinel && virtualenv ./venv && ./venv/bin/pip install -r requirements.txt", shell=True)
    cron = CronTab(user='root')
    cron.new(command='cd /root/geek-sentinel && ./venv/bin/python bin/sentinel.py >/dev/null 2>&1', comment='Sentinel for GeekCash')
    cron.write()

...
port = "13058"
rpcuser = "redenuser"
datadir = "/root/.redencore"
config_file = "reden.conf"
cli = "reden-cli"
coind = "redend"
walletUrl = "https://github.com/RedenCore/Reden/releases/download"
walletFile = "reden-%s-ubuntu16.tar.gz" % (walletVer)
ticker = "REDN"
...
def removeOldFiles():
    ...
    subprocess.call("rm -rf %s /usr/local/bin/reden*" % (datadir), shell=True)
...
def procFile(data):
    ...
    subprocess.call("mv /tmp/reden-%s-ubuntu16/reden/* /usr/local/bin/" % (walletVer), shell=True)

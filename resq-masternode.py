...
port = "19988"
rpcuser = "resquser"
datadir = "/root/.resqcore"
config_file = "resq.conf"
cli = "resq-cli"
coind = "resqd"
walletUrl = "https://github.com/RESQ-Chain/resq-chain/releases/download/v%s" % (walletVer)
walletFile = "resq-linux.tar.gz"
ticker = "RESQ"
...
def removeOldFiles():
    ...
    subprocess.call("rm -rf %s /usr/local/bin/resq*" % (datadir), shell=True)
...
def procFile(data):
    ...
    subprocess.call("mv /tmp/resq/* /usr/local/bin/", shell=True)    
    subprocess.call("chmod +x /usr/local/bin/resq*", shell=True)

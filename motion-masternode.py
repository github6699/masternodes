...
port = "7979"
rpcuser = "motionuser"
datadir = "/root/.motioncore"
config_file = "motion.conf"
cli = "motion-cli"
coind = "motiond"
walletUrl = "https://github.com/motioncrypto/motion/releases/download"
walletFile = "motion-%s-lin-64bits.zip" % (walletVer)
ticker = "XMN"
...
def installUbuntuPackages():
    ...
    subprocess.call("apt-get -y install wget git dnsutils psmisc libssl-dev python-pip libzmq3-dev libevent-dev bsdmainutils software-properties-common libboost-all-dev libminiupnpc-dev libdb4.8-dev libdb4.8++-dev", shell=True)
...
def removeOldFiles():
    ...
    subprocess.call("rm -rf %s /usr/local/bin/motion*" % (datadir), shell=True)
...
def downloadWallet():
    ...
    ## Download wallet
    url = "%s/%s/%s" % (walletUrl, walletVer, walletFile)
    file_name = "wallet"
...
def procFile(data):
    ...
    subprocess.call("mv /tmp/%s /tmp/%s /usr/local/bin/" % (coind, cli), shell=True)
...
def installSentinel():
    ...
    subprocess.call("git clone https://github.com/motioncrypto/sentinel.git /root/motion-sentinel", shell=True)
    subprocess.call("cd /root/motion-sentinel && virtualenv ./venv && ./venv/bin/pip install -r requirements.txt", shell=True)
    cron = CronTab(user='root')
    cron.new(command='cd /root/motion-sentinel && ./venv/bin/python bin/sentinel.py >/dev/null 2>&1', comment='Sentinel for Motion')
    cron.write()
